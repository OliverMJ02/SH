...
{
    public class SuperSetBlock : ObservableObject
    {
        ...
        private bool superSetBlockVisibility;
        public bool SuperSetBlockVisibility
        {
            get { return superSetBlockVisibility; }
            set
            {

                superSetBlockVisibility = value;
                OnPropertyChanged(nameof(SuperSetBlockVisibility));
            }
        }
        ...
        private string AexerciseTitle;  // The title of the current exercise chosen

        public string AExerciseTitle
        {
            get { return AexerciseTitle; }
            set
            {
                AexerciseTitle = value;
                OnPropertyChanged(nameof(AExerciseTitle));
            }
        }

        private string BexerciseTitle;  // The title of the current exercise chosen

        public string BExerciseTitle
        {
            get { return BexerciseTitle; }
            set
            {
                BexerciseTitle = value;
                OnPropertyChanged(nameof(BExerciseTitle));
            }
        }
        ...
        //--
        [OneToMany]
        public List<SuperSet> SuperSets { get; set; } // List of sets connected to the block
        ...
        public SuperSetBlock()
        {
            ...
            HandleMinChangeCmd = new Command<(int, int, IList<int>)>(tuple =>
            {
                ...
                Width = 0;
                XMargin = 40;
                App.Database.UpdateSuperBlockWithChildren(this);
            });
            ...
            UpdateBlockVisibilityCmd = new Command(() => { UpdateSuperSetBlockVisibility(); IsNotFinished = false; SaveOrFinish = "SAVE CHANGES"; App.Database.UpdateSuperBlockWithChildren(this); });
            ...
            HandleRestingTimerCmd = new Command(() => {
                ...
                int c = 0;
                TimerOn = App.Database.GetSuperBlockWithChildren(this.Id).TimerOn;
                if (TimerOn)
                {
                    TimerOn = false;
                    App.Database.UpdateSuperBlockWithChildren(this);
                ...
                else
                {
                    ...
                    Device.StartTimer(TimeSpan.FromMilliseconds(50), () =>
                    {
                        ...
                        try { return App.Database.GetSuperBlockWithChildren(this.Id).TimerOn; }
                        catch { return false; }
        ...
        public SuperSetBlock(int Counter)
        {
            IsSuperSet = true;
            IsNotFinished = true;
        ...
        public SuperSetBlock CloseAllSets()
        {
            foreach (SuperSet SS in this.SuperSets)
            {
                SS.AIsVisible = false;
                SS.AIsVisibleSource = "pointer_down_gray.png";
                SS.BIsVisible = false;
                SS.BIsVisibleSource = "pointer_down_gray.png";
                App.Database.UpdateSuperSetWithChildren(SS);
            }
            return this;
        }

        public void UpdateSuperSetTitels(int i)
        {
            foreach (SuperSet ss in this.SuperSets)
            {
                if (int.Parse((ss.Title)) > i)
                {
                    ss.Title = (int.Parse((ss.Title)) - 1).ToString();
                    App.Database.UpdateSuperSetWithChildren(ss);
                }
            }
        }

        public SuperSetBlock UpdateSuperSetBlockVisibility()
        {
            if (this.SuperSetBlockVisibility)
            {
                this.SuperSetBlockVisibility = false;
            }
            else
            {
                this.SuperSetBlockVisibility = true;
            }
            return this;
        }
    }
